...
from ..utils import get_order_or_404
...
def get_order_details(id: int,
    ...
    order = get_order_or_404(id, current_user.id, session)
    return order
...
def cancel_order(id: int,
    ...
    if order.status == OrderStatus.CANCELED:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=f"Order with ID:{id} already cancelled")
...
def return_order(id: int,
    ...
    order_found.status = OrderStatus.RETURNED
